...
interactions = get_data_parquet(interactions_link)
movies_md = get_data_parquet(movies_md_link)
movies_rd = get_data_xslx(movies_rd_link)

movies_rd = preprocess_movies_rd(movies_rd)

save_data_parquet(movies_rd_upd_link,movies_rd)

int_coef = get_int_coef(interactions)

save_data_parquet(int_coef_link,int_coef)
...
save_data_parquet(movies_md_upd_link,movies_upd)
save_data_parquet(interactions_upd_link,interactions)

cos_sim_md = get_cosine_mat(movies_upd,'desc_md')
save_matrix(cos_sim_md_link,cos_sim_md)


cos_sim_desc = get_cosine_mat(movies_upd,'description')
save_matrix(cos_sim_desc_link,cos_sim_desc)
